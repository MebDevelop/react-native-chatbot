...
import { Dimensions, Keyboard, TextInput, ScrollView, Platform, Image } from 'react-native';
...
class ChatBot extends Component {
  ...
  componentWillMount() {
    ...
    const firstStep = this.props.steps.filter(each => each.id.toString() === startStep.toString())[0];
  ...
  triggerNextStep(data) {
    ...
    // add triggerStep props
    // this.props.triggerStep(currentStep.id);
    ...
    if (isEnd) {
      ...
      this.props.triggerStep(currentStep.trigger);
  ...
  renderStep(step, index) {
    ...
    const { options, component, asMessage, imageUrl, width, height } = step;
    const steps = {};
...
ChatBot.defaultProps = {
  ...
  placeholder: 'Tap here to reply to Jimi',
